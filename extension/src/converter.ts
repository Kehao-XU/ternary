...
class BigFraction {
    readonly num: bigint;  // 分子（可负）
    readonly den: bigint;  // 分母（恒正）
    ...
    eq(other: BigFraction): boolean {
        return this.num === other.num && this.den === other.den;
    }

    lt(other: BigFraction): boolean {
        return this.num * other.den < other.num * this.den;
    }

    gt(other: BigFraction): boolean {
        return this.num * other.den > other.num * this.den;
    }

    lte(other: BigFraction): boolean {
        return this.num * other.den <= other.num * this.den;
    }

    gte(other: BigFraction): boolean {
        return this.num * other.den >= other.num * this.den;
    }
...
function parseToFraction(s: string): BigFraction {
    s = s.trim();
    ...
    if (s.includes('/')) {
        const parts = s.split('/');
        if (parts.length !== 2) { throw new Error(`无效的分数：${s}`); }
 ...
 */
export function decimalToBalancedTernary(value: string, maxDigits: number = 2000): string {
    if (maxDigits <= 0) { throw new Error('maxDigits 必须为正整数'); }
    ...
    if (sign < 0) { result = negateBT(result); }

    return result;
}

/** 非负整数 → 平衡三进制 */
function integerToBT(n: bigint): string {
    if (n === 0n) { return '0'; }
...
function fractionToBT(f: BigFraction, maxDigits: number): string {
    if (f.isZero()) { return ''; }

    const seen = new Map<string, number>();  // remainder key → 位置
    const digits: string[] = [];
...
/** 平衡三进制取反：T ↔ 1 */
function negateBT(s: string): string {
    let result = '';
    for (const ch of s) {
        if (ch === 'T') { result += '1'; }
        else if (ch === '1') { result += 'T'; }
        else if (ch === '0') { result += '0'; }
        else { result += ch; }  // '.', '(', ')' 等保持原样
    }
    return result;
}
 ...
 * @returns 十进制字符串
 */
export function balancedTernaryToDecimal(s: string): string {
    ...
    if (fracRaw.includes('(')) {
        ...
        if (!match) { throw new Error(`循环节格式错误：${fracRaw}`); }
        prefix = match[1];
        cycle = match[2];
        validateBT(prefix);
        validateBT(cycle);
    ...
    for (let i = 0; i < intPart.length; i++) {
        ...
        if (ch === '1') { value = value.add(weight); }
        else if (ch === 'T') { value = value.sub(weight); }
    }
...
function evalFracBT(digits: string, startPos: number): BigFraction {
    let val = ZERO;
    for (let i = 0; i < digits.length; i++) {
        const pos = startPos + i;
        const weight = new BigFraction(1n, 3n ** BigInt(pos));
        if (digits[i] === '1') { val = val.add(weight); }
        else if (digits[i] === 'T') { val = val.sub(weight); }
    }
    return val;
}

/** Fraction → 十进制字符串（长除法，检测循环节） */
function fractionToDecimalString(f: BigFraction): string {
    if (f.isZero()) { return '0'; }
    ...
    const digits: string[] = [];
    const seen = new Map<string, number>();  // remainder → position
...
function validateBT(s: string): void {
    for (const ch of s) {
        if (ch !== 'T' && ch !== '0' && ch !== '1') {
...
export function isBalancedTernary(s: string): boolean {
    s = s.trim().toUpperCase();
