...
import { decimalToBalancedTernary, balancedTernaryToDecimal } from './converter';

export class CalculatorPanel {
    private static currentPanel: CalculatorPanel | undefined;
    ...
    private _handleMessage(msg: { id: string; type: string; value: string }) {
        try {
            let result: string;
            switch (msg.type) {
                case 'toTernary':
                    result = decimalToBalancedTernary(msg.value);
                    break;
                case 'toDecimal':
                    result = balancedTernaryToDecimal(msg.value);
                    break;
                default:
                    return;
            }
